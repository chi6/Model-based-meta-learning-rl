...
class VectorizedSampler(BaseSampler):
    ...
    def get_random_action(self, observation, idx=None):
        ...
        mean = np.random.uniform(low=-1.0, high= 1.0, size= [self.n_envs,self.algo.a_size])
    ...
    def get_MPC_action(self, obs):
        ...
        return np.asarray(action_list).reshape([-1, self.algo.env.action_space.shape[0]]), dict(means = action_list)
