...
class BatchMAMLPolopt(RLAlgorithm):
    ...
    def __init__(
        ...
        self.goal = np.ones([self.meta_batch_size],dtype=np.float32)*2.0
        ...
        self.test_sampler = VectorizedSampler(self,n_envs=1)
    ...
    def MPC(self, num_samples):
        ...
        for i in range(num_samples):
            inputs = ext.extract(samples_data[i],
                                 'observations', 'actions', 'advantages')
            # inputs_list.append(np.concatenate((inputs[0], inputs[1]), axis = 1).astype(np.float32))
            obs_list.append(inputs[0])
            action_list.append(inputs[1].reshape([-1, 20 ,self.env.action_space.flat_dim]))
            adv_list.append(0)
        ...
        for i in range(num_samples):
            ...
            for j in range(action_list[i].shape[0]):
                ...
                action = np.clip(action_list[i][j], *self.env.action_space.bounds)
                _, reward, _, _ = self.env.step(action)
                adv_list[i] = adv_list[i] + reward
                new_obs_list.append(self.policy.get_state(obs_list[i][j], action_list[i][j]))
            obs_list[i] = new_obs_list

        index = np.argmax(adv_list)
        return action_list[index][0]

    def meta_online_train(self):
        ...
        with tf.Session() as sess:
            ...
            for itr in range(self.start_itr, self.n_itr):
                ...
                obs, act, adv = [], [], []
                hack_obs, hack_act, hack_adv = [], [], []
                for step in range(self.max_path_length):
                    if step == 0:
                        init_state = self.env.reset()
                        print("Doing MPC, step:", step)
                        action = self.MPC(num_samples= self.meta_batch_size)
                        action = np.clip(action, *self.env.action_space.bounds)
                        new_obs, reward, _ ,_= self.env.step(action)
                        obs.append(init_state.reshape([-1, 20]))
                        act.append(action.reshape([-1, 120]))
                        adv.append(reward.reshape([-1]))

                        hack_obs, hack_act, hack_adv = [], [], []
                        for k in range(self.meta_batch_size):
                            hack_obs.append(np.asarray(obs).reshape([-1, 20]))
                            hack_act.append(np.asarray(act).reshape([-1,120]))
                            hack_adv.append(np.asarray(adv).reshape([-1]))

                        init_state = new_obs


                    input_list = hack_obs + hack_act + hack_adv

                    self.policy.compute_updated_dists(samples=input_list, process_data = False)
                    print("Doing MPC, step:", step)
                    action = self.MPC(num_samples= self.meta_batch_size)
                    action = np.clip(action, *self.env.action_space.bounds)
                    new_obs, reward, _ ,_= self.env.step(action)
                    obs.append(init_state)
                    act.append(action)
                    init_state = new_obs
                    logger.log("Saving snapshot...")
                    params = self.get_itr_snapshot(itr, obs)  # , **kwargs)
        ...
        with tf.Session() as sess:
            ...
            for itr in range(self.start_itr, self.n_itr):
                ...
                with logger.prefix('itr #%d | ' % itr):
                    ...
                    all_samples_data, all_future_samples_data, all_paths, all_future_paths = [], [], [], []
                    ...
                    #    import pdb; pdb.set_trace() # test param_vals functions.
                        ...
                        #logger.log("Obtaining future samples...")
                        #future_paths = self.obtain_samples(itr, reset_args=learner_env_goals, log_prefix=str(step))
                        all_paths.append(paths)
                        #all_future_paths.append(future_paths)
                        ...
                        for key in paths.keys():  # the keys are the tasks
                            ...
                            #future_samples_data[key] = self.process_samples(itr, future_paths[key], log = False)
                        all_samples_data.append(samples_data)
                        #all_future_samples_data.append(future_samples_data)
                        # for logging purposes only
                        self.process_samples(itr, flatten_list(paths.values()), prefix=str(step), log=True)
                        #logger.log("Logging diagnostics...")
                        #self.log_diagnostics(flatten_list(paths.values()), prefix=str(0))
                        if step == 0:
                    ...
                    if True and itr % 2 == 0 and self.env.observation_space.shape[0] <= 4: # point-mass
                    ...
                    elif True and itr % 2 == 0:  # swimmer or cheetah
                        logger.log("Saving visualization of paths")
                        for ind in range(min(5, self.meta_batch_size)):
                            plt.clf()
    ...
    def optimize_policy(self, itr, samples_data, process_data = True):
        raise NotImplementedError
