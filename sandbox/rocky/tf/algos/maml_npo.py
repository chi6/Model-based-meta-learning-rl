...
class MAMLNPO(BatchMAMLPolopt):
    ...
    def make_vars(self, stepnum='0'):
        ...
        for i in range(self.meta_batch_size):
            ...
            action_vars.append(tf.placeholder(tf.float32, shape=[None]  + [self.env.action_space.flat_dim * 20], name='action' + stepnum + '_' + str(i)))
    ...
    def init_opt(self):
        ...
        for j in range(self.num_grad_updates):
            ...
            for i in range(self.meta_batch_size):
                input_vars = tf.concat([obs_vars[i], action_vars[i]],  1)
                if j == 0:
                    # j == 0, first gradient update, doing optimization on original parameters
                    dist_info_vars, params = self.policy.dist_info_sym(input_vars, state_info_vars, all_params=self.policy.all_params)
                    if self.kl_constrain_step == 0:
                        kl = dist.kl_sym(old_dist_info_vars[i], dist_info_vars)
                        kls.append(kl)
                else:
                    # j != 0, do more gradient update optimization
                    dist_info_vars, params = self.policy.updated_dist_info_sym(i, all_surr_objs[-1][i], input_vars, params_dict=cur_params[i])

                new_params.append(params)
                #logli = dist.log_likelihood_sym(action_vars[i], dist_info_vars)
                #diff = obs_vars[i][1:-1] - obs_vars[i][0:-2]
                diff = obs_vars[i][1:-1] - obs_vars[i][0:-2]

                # formulate as a minimization problem
                # The gradient of the surrogate objective is the policy gradient
                surr_objs.append( tf.reduce_mean(tf.squared_difference(diff, dist_info_vars[0:-2])))
        ...
        for i in range(self.meta_batch_size):
            input_vars = tf.concat([obs_vars[i], action_vars[i]], 1)
            dist_info_vars, _ = self.policy.updated_dist_info_sym(i, all_surr_objs[-1][i], input_vars, params_dict=new_params[i])
            ...
            diff = obs_vars[i][1:-1] - obs_vars[i][0:-2]
            surr_objs.append(tf.reduce_mean(tf.squared_difference(diff, dist_info_vars[0:-2])))
            #new_input_list.append(input_vars)
        ...
        if self.use_maml:
            mean_kl = tf.constant(0, dtype= tf.float32) #tf.reduce_mean(tf.concat(kls, 0))  ##CF shouldn't this have the option of self.kl_constrain_step == -1?
    ...
    @overrides
    def optimize_policy(self, itr, all_samples_data, process_data = True):
        ...
        if process_data:
            ...
            input_list = all_samples_data

        '''if self.use_maml:
            dist_info_list = []
            for i in range(self.meta_batch_size):
                agent_infos = all_samples_data[i]['agent_infos']
