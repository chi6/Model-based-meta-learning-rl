...
class MAMLGaussianMLPPolicy(StochasticPolicy, Serializable):
    def __init__(
        ...
        self.input_shape = (None, obs_dim + self.action_dim*20 ,)
        ...
        if mean_network is None:
            ...
            self.input_tensor, self.output_tensor = self.forward_MLP('mean_network', self.all_params,
    ...
    def compute_updated_dists(self, samples, process_data = True):
    ...
    def dist_info_sym(self, obs_var, state_info_vars=None, all_params=None, is_training=True):
        ...
        mean_var = self._forward(obs_var, all_params, is_training)
        '''
        ...
        if self.std_parametrization == 'exp':
            ...
            raise NotImplementedError
        '''
        log_std_var = tf.constant(0, dtype=tf.float32)
        if return_params:
            return  mean_var, all_params # dict(mean=mean_var, log_std=log_std_var), all_params
        else:
    ...
    def get_state(self, obs_list, act_list):

        input_list = np.concatenate((obs_list,act_list.flatten()))[np.newaxis, :]
        state_diff = self._cur_f_dist(input_list)
        new_state = state_diff + obs_list
    ...
    def get_action(self, observation, idx=None):
        ...
        mean = np.random.uniform(low=-1.0, high= 1.0, size= [flat_obs.shape[0],6])
    ...
    def get_actions(self, observations):
        ...
        flat_obs = self.observation_space.flatten(observations)
        #f_dist = self._cur_f_dist
        mean = np.random.uniform(low=-1.0, high= 1.0, size= [flat_obs.shape[0],6])
        #mean, log_std = [x[0] for x in f_dist([flat_obs])]
