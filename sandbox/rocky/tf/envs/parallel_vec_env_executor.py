...
def worker_run_reset(G, flags, scope, reset_args):
    if not hasattr(G, 'parallel_vec_envs'):
    ...
    obs_dim = env_template.observation_space.shape[0]
    ...
    for itr_idx, (idx, env) in enumerate(G.parallel_vec_envs[scope]):
        flag = flags[idx]
        if flag:
            flat_obs.append(env.reset(reset_args = None))
            reset_ids.append(itr_idx)
        ids.append(idx)
    if len(reset_ids) > 0:

        ret_arr[reset_ids] = flat_obs #env_template.observation_space.flatten_n(flat_obs)
    return ids, ret_arr


def worker_run_step(G, action_n, scope,reset_args = None):
    assert hasattr(G, 'parallel_vec_envs')
    assert scope in G.parallel_vec_envs
    env_template = G.parallel_vec_env_template[scope]
    ids = []
    ...
    obs = obs#env_template.observation_space.flatten_n(obs)
...
class ParallelVecEnvExecutor(object):
    ...
    def step(self, action_n, reset_args = None):
        results = singleton_pool.run_each(
            worker_run_step,
            [(action_n, self.scope, reset_args) for _ in self._alloc_env_ids],
        )
        results = [x for x in results if x is not None]
        ids, obs, rewards, dones, env_infos = list(zip(*results))
        ids = np.concatenate(ids)
        obs = np.concatenate(obs)#self.observation_space.unflatten_n(np.concatenate(obs))
        ...
        reset_obs = self._run_reset(dones, self.reset_args)
    ...
    def _run_reset(self, dones, reset):
        dones = np.asarray(dones)
        reset = np.asarray(reset)

        results = singleton_pool.run_each(
            worker_run_reset,
            [(dones, self.scope, reset) ]#for i in range(reset.shape[0])],
        )
        ...
        done_unflat_obs = done_flat_obs# self.observation_space.unflatten_n(done_flat_obs)
    ...
    def reset(self,reset_args):
        ...
        reset = np.asarray(self.reset_args)

        return self._run_reset(dones, reset)
